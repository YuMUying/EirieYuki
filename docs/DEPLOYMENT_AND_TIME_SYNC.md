...
视觉设备运行两个独立常驻 ONNX 工作节点：探头相机优先 30 Hz，顶部相机 12 Hz。采用深度为一的最新帧队列，过载时丢弃旧帧而不是积累延迟；详细话题、基准与验收见 [双相机实时部署](REALTIME_VISION_DEPLOYMENT.md)。
...
8. 连续记录两路 `VisionTiming`，核验处理 P95、端到端 P95/P99、有效频率和丢帧数。
9. 只有 `timing_valid`、视觉 deadline、标定和失效测试均通过后，才允许自动焊缝跟踪和空间成像。
